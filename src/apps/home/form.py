from typing import Any
from django.contrib.auth import forms
from apps.home.models import User
from django import forms as form_data


class Create_user_form(forms.UserCreationForm):
    error_css_class = "is-invalid"
    required_css_class = "required-field"

    class Meta:
        model = User

        fields = ("username", "nombre", "apellido", "dni", "edad")

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)

    def is_valid(self) -> bool:
        validated = super().is_valid()
        self.error_messages = {}
        # c1  = User.apellido
        # c2  = (getattr(User, "apellido"))

        # print(c2)
        field_list = []
        for field in self.fields.keys():
            field_list.insert(len(field_list), str(field))

        for field in field_list:
    
            try:
                if (
                    self.cleaned_data[field] == None
                    
                ):
                    # print("-------------------------------")
                    # print(field)
                    # print("-------------------------------")
                    self.error_messages.update(
                        {field: field.upper() + " invalido o ya registrado."}
                    )
            except:
                self.error_messages.update(
                    {field: field.upper() + " invalido o ya registrado."}
                )

        try:
            if self.cleaned_data["password1"] != self.cleaned_data["password2"]:
                self.error_messages.update(
                    {"password": "Contraseñas invalidas o no coinciden."}
                )
        except:
            self.error_messages.update(
                {"password": "Contraseñas invalidas o no coinciden."}
            )

        # try:

        # except:

        # for key in self.cleaned_data:

        #     if self.cleaned_data[key] == None:
        #         print(key)
        #         self.error_messages.update(
        #             {str(key): str(key).capitalize() + " invalido o ya registrado."}
        #         )

        # try:
        #     if self.cleaned_data["password1"] != self.cleaned_data["password2"]:
        #         self.error_messages.update(
        #             {"password": "Contraseñas invalidas o no coinciden."}
        #         )
        # except:
        #     self.error_messages.update(
        #         {"password": "Contraseñas invalidas o no coinciden."}
        #     )
        return validated


class Login_User_Form(forms.AuthenticationForm, form_data.Form):
    error_css_class = "is-invalid"
    required_css_class = "required-field"
    username = form_data.CharField(label="Nombre de Usuario")
    password = form_data.CharField(label="Contraseña", widget=form_data.PasswordInput)
